...
async function generateProductPage() {
  ...
  const productId = await question(
    'Product ID (URL slug, e.g., "oqsx-g2-oil-quality-sensor"): ',
  );
  const productName = await question(
    'Product Name (e.g., "OQSx-G2 Oil Quality Sensor"): ',
  );
  ...
  while (addMoreImages) {
    const imageSrc = await question(
      `Image ${imageIndex} path (relative to public/): `,
    );
  ...
  while (addMoreParagraphs) {
    const paragraph = await question(
      `Description paragraph ${paragraphIndex}: `,
    );
  ...
  const breadcrumbItems = [
    ...
    {
      label: capitalizeWords(category.replace("-", " ")),
      href: `/products/${category}`,
    },
    {
      label: capitalizeWords(subcategory.replace("-", " ")),
...
function generateProductDataEntry(productData) {
  const imagesStr = productData.images
    .map(
      (img) =>
        `    {\n      src: "${img.src}",\n      alt: "${img.alt}"\n    }`,
    )
  ...
  const descriptionStr = productData.description
    .map((desc) => `    "${desc}"`)
    .join(",\n");

  const breadcrumbStr = productData.breadcrumbItems
    .map((item) =>
      item.href
