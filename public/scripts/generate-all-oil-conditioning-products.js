...
const productsDataPath = path.join(
  __dirname,
  "../src/lib/oil-conditioning-products.ts",
);
...
function createProductPage(productId, productDataVar, productDataName) {
  ...
  if (
    productId.includes("tandelta") ||
    productId.includes("oqsx") ||
    productId.includes("mot") ||
    productId.includes("sense") ||
    productId.includes("gateway") ||
    productId.includes("manifold") ||
    productId.includes("cable") ||
    productId.includes("oqde")
  ) {
    subcategory = "tandelta";
  } else if (
    productId.includes("filtertechnik") ||
    productId.includes("particle") ||
    productId.includes("s120") ||
    productId.includes("pal")
  ) {
    subcategory = "filtertechnik";
  } else if (
    productId.includes("hydrotechnik") ||
    productId.includes("watchlog") ||
    productId.includes("wlb") ||
    productId.includes("wlw") ||
    productId.includes("ht")
  ) {
  ...
  fs.writeFileSync(
    pagePath,
    productPageTemplate(productDataVar, productDataName),
  );
