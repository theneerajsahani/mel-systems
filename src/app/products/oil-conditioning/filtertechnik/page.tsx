...
export default function FiltertechnikPage() {
    ...
    const filteredProducts = filtertechnikProducts.filter(product => {
        const matchesSearch = searchTerm === '' || 
            ...
            (product.description && product.description.join(' ').toLowerCase().includes(searchTerm.toLowerCase())) ||
                ...
                {viewMode === 'grid' && (
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                        {filteredProducts.map((product) => (
                            <Card key={product.id} className="overflow-hidden group shadow-md border-0">
                                <Link href={`${product.href ?? `/products/oil-conditioning/filtertechnik/${product.id}` ?? "#"}`} className="block">
                                    ...
                                    <div className="p-4">
                                        ...
                                        <p className="text-sm text-muted-foreground mb-2">{product.description && product.description[0]}</p>
